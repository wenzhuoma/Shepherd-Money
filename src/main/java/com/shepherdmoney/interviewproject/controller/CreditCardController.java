...
public class CreditCardController {
     ...
     *
     * @param payload AddCreditCardToUserPayload
     * @return response
     */
    ...
    public ResponseEntity<List<CreditCardView>> getAllCardOfUser(@RequestParam int userId) {
        ...
        List<CreditCardView> creditCardViews = creditCradsList.stream().map(creditCard ->
                new CreditCardView(creditCard.getIssuanceBank(), creditCard.getNumber())).toList();
    ...
    public ResponseEntity<Void> updateBalancesOfCreditCard(@RequestBody UpdateBalancePayload[] payload) {
        try {
            ...
            for (UpdateBalancePayload updateBalance : payload) {
                ...
                Optional<CreditCard> currentCreditCard = creditCardRepository.
                        findByNumber(updateBalance.getCreditCardNumber());
                ...
                Optional<BalanceHistory> currentBalanceHistory = creditCard.getBalanceHistories().stream().
                        filter(history -> history.getDate().equals(updateBalance.getTransactionTime())).findFirst();

                if (currentBalanceHistory.isEmpty()) {
                    // no balance history on the same date, add the balance history
                    BalanceHistory balanceHistory = new BalanceHistory(updateBalance.getTransactionTime(),
                            updateBalance.getTransactionAmount(), creditCard);
                    balanceHistoriesList.add(balanceHistory);
                } else {
                    //otherwise, update an existing one
                    currentBalanceHistory.get().setBalance(currentBalanceHistory.get().getBalance() + updateBalance.
            ...
            //return 400 OK if  it has exception
            return ResponseEntity.badRequest().build();
        }
